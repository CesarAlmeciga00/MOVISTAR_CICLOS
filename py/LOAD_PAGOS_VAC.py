...
class pagosVac:
    def loadPagosVac(ip, user_, password, bbdd):
        start_time = time.time()
        try:
            connection = MySQLdb.Connect(host=ip, user=user_, passwd=password, db=bbdd)
            cursor = connection.cursor()
            try:
                truncate = "TRUNCATE bbdd_cs_bog_movistar_ciclos.tb_pagos_vac_movistar;"
                cursor.execute(truncate)
                connection.commit()
                set1 = "set global unique_checks = 0;"
                set2 = "set global foreign_key_checks = 0;"
                disable_key1 = "ALTER TABLE tb_pagos_vac_movistar drop index id_cliente;"
                disable_key2 = "ALTER TABLE tb_pagos_vac_movistar drop index id_fecha;"
                cursor.execute(set1)
                cursor.execute(set2)
            ...
            try:
                query = """LOAD DATA LOCAL INFILE 'C:/MOVISTAR_COLOMBIA/02_CRUDOS/PAGOS/PAGOS_VAC.txt' 
                            INTO TABLE
                            tb_pagos_vac_movistar 
                ...
                enable_key1 = "ALTER TABLE tb_pagos_vac_movistar ADD INDEX `id_cliente` USING BTREE (`cod_cliente`) VISIBLE;"
                enable_key2 = "ALTER TABLE tb_pagos_vac_movistar ADD INDEX `id_fecha` USING BTREE (`fecha_pago` DESC) VISIBLE;"
