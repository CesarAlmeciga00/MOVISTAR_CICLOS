...
class email:
    ...
    def loadEmail(connection):
        ...
        try:
            ...
            xlsx = pd.ExcelFile('EMAILS.xlsx', engine='openpyxl')
            df_masivos = xlsx.parse(index_col=None)
            df_masivos  = df_masivos.iloc[: , :27]
        ...
        try:
            ...
            for index, row in delete_df.iterrows():
                query_delete = """DELETE FROM `bbdd_cs_bog_movistar_ciclos`.`tb_masivos_email_movistar_ciclos`
        ...
        try:
            ...
            df_masivos.to_sql("tb_masivos_email_movistar_ciclos", connection, if_exists = 'append', index=None, chunksize=10000)
        except:
