  ...
  def loadCarteras(ip, user_, password, bbdd):
    ...
    try:
      ...
      try:  
        ...
        enable_key = "ALTER TABLE `bbdd_cs_bog_movistar_ciclos`.`tb_cartera_fs_movistar_ciclos` ADD INDEX `id_cuenta` USING BTREE (`CUENTA_FS`, `CARTERA_SERVICIO`, `DIAS_MORA_SERVICIO`, `EDAD_MORA_SERVICIO`) VISIBLE;"
        cursor.execute(set1)
        cursor.execute(set2)
